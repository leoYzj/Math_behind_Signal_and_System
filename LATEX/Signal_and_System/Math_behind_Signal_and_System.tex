...
\lr{
    (\mathcal{F} f)^-(\xi)&=\int_{-\infty}^{\infty}f(t)e^{2\pi i \xi t}\,dt\\
    \mathcal{F} (f^-)(\xi)&=\int_{-\infty}^{\infty}f(-t)e^{-2\pi i \xi t}\,dt\\
    &=\int_{-\infty}^{\infty}f(t)e^{2\pi i \xi t}\,dt\text{（变量代换）}\\
    \mathcal{F} ^{-1}f(x)&=\int_{-\infty}^{\infty}f(t)e^{2\pi ixt}\,dt\\
    \text{第一行得证。}&\text{同时取FT，即得}\mathcal{F} \mathcal{F} f=f^-\\
    ...
    (\mathcal{F} f)^-(\omega)&=\int_{-\infty}^{\infty}f(t)e^{i \omega t}\,dt\\
    \mathcal{F} (f^-)(\omega)&=\int_{-\infty}^{\infty}f(-t)e^{-i \omega t}\,dt\\
    &=\int_{-\infty}^{\infty}f(t)e^{i \omega t}\,dt\text{（变量代换）}\\
    \mathcal{F} ^{-1}f(x)&=\frac{1}{2\pi}\int_{-\infty}^{\infty}f(t)e^{i\omega t}\,dt\\
    \text{第一行得证。}&\text{同时取FT，即得}\mathcal{F} \mathcal{F} f=2\pi f^-\\
    \text{f是实信号时，}&f=\overline{f},\\
    \mathcal{F} f^-(\omega)&=\int_{-\infty}^{\infty}f(t)e^{i \omega t}\,dt\\
...
&=e^{2\pi i \xi b}\int_{-\infty}^{\infty}f(t)e^{-2\pi i \xi t}\,dt=e^{2\pi i \xi b}\mathcal{F} f(\xi)
}{
\mathcal{F} [f(t-b)](\omega)&=\int_{-\infty}^{\infty}f(t-b)e^{-i \omega t}\,dt\\
&=\int_{-\infty}^{\infty}f(t)e^{-i \omega (t+b)}\,dt\\
&=e^{i \omega b}\int_{-\infty}^{\infty}f(t)e^{-i \omega t}\,dt=e^{i \omega b}\mathcal{F} f(\omega)
}
\noindent 5.伸缩定理\lr{
...
注意变量代换时，如果a<0，积分上下限也会改变，这正是绝对值的来源。对此有疑惑的读者可以自行分情况验算。\\
6.微分性质
\lr    {
\mathcal{F} (f')(\xi)&=\int_{-\infty}^{\infty}f'(t)e^{-2\pi i\xi t}\,dt\\
&=\int_{-\infty}^{\infty}e^{-2\pi i\xi t}\,df(t)\\
&=e^{-2\pi i\xi t}f(t)|_{t=-\infty}^{\infty}+2\pi i\xi\int_{-\infty}^{\infty}f(t)e^{-2\pi i\xi t}\,dt\\
...
&=e^{-i\omega t}f(t)|_{t=-\infty}^{\infty}+i\omega\int_{-\infty}^{\infty}f(t)e^{-i\omega t}\,dt\\
...
$e^{-2\pi i\xi t}f(t)|_{t=-\infty}^{\infty},e^{-i\omega t}f(t)|_{t=-\infty}^{\infty}=0$
...
&=-\frac{E}{2\pi i\xi}e^{-2\pi i\xi t}|_{t=-\frac{T}{2}}^{\frac{T}{2}}\\
...
&=-\frac{E}{i\omega}e^{-i\omega t}|_{t=-\frac{T}{2}}^{\frac{T}{2}}\\
...
叶级数的深刻关系。在此之前，不仅是1，正弦、余弦函数以及所有的周期函数也无法做傅里叶变换。\\
...
&=F(1)-F(-1)-\frac{1}{2\pi i\xi}(te^{-2\pi i\xi t}|_{-1}^0\\
&\quad -\int_{-1}^{0}e^{-2\pi i\xi t}\,dt-te^{-2\pi i\xi t}|_0^1+\int_{0}^{1}e^{-2\pi i\xi t}\,dt)\\
&=\frac{F(1)-2F(0)+F(-1)}{2\pi i\xi}\\
&=\frac{e^{2\pi i\xi}+e^{-2\pi i\xi}-2}{(2\pi i\xi)^2}=\frac{1}{(\pi\xi)^2}(\frac{e^{\pi\xi}-e^{-\pi\xi}}{2i})^2\\
&=sinc^2(\xi)
}{
&\text{记$e^{2\pi i\xi t}$的原函数为}F(t)=\frac{e^{i\omega t}}{i\omega}\\
\mathcal{F} \Lambda(\omega)&=\int_{-\infty}^{\infty}\Lambda(t)e^{-i\omega t}\,dt\\
&=\int_{-1}^{0}(1+t)e^{-i\omega t}\,dt+\int_{0}^{1}(1-t)e^{-i\omega t}\,dt\\
&=F(1)-F(-1)-\frac{1}{i\omega}(te^{-i\omega t}|_{-1}^0\\
&\quad -\int_{-1}^{0}e^{-i\omega t}\,dt-te^{-i\omega t}|_0^1+\int_{0}^{1}e^{-i\omega t}\,dt)\\
...
\begin{figure}
    ...
    \caption{$\Lambda$函数图像}
\end{figure}
\textbullet 高斯函数 $G(t)=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{t^2}{2\sigma^2}}$，求它的傅里叶变换的方法较为特殊：
\lr{
...
\begin{figure}[htbp]
    ...
    \caption{高斯函数图像}
\end{figure}
\textbullet 单边指数函数$f(t)=\begin{cases}
        e^{-at}, & \text{if }t\geq 0 \\
        0,       & \text{if }t<0
...
&=-\frac{1}{a+2\pi i\xi}e^{-(a+2\pi i\xi)t}|_{0}^{\infty}\\
&=\frac{1}{a+2\pi i\xi}
}{
\mathcal{F} f(\omega)&=\int_{-\infty}^{\infty}f(t)e^{-i\omega t}\,dt\\
&=\int_{0}^{\infty}e^{-at}e^{-i\omega t}\,dt\\
&=-\frac{1}{a+i\omega}e^{-(a+i\omega)t}|_{0}^{\infty}\\
&=\frac{1}{a+i\omega}
}
运用对偶性，立即得到
\lr{
...
\lr{
    ...
    \mathcal{F} (fg)(\omega)=2\pi(\mathcal{F} f*\mathcal{F} g)(\omega)
}
...
根据傅里叶反演公式，
\lr{
\mathcal{F} \mathds{1}(\xi)&=\delta(\xi)\\
\mathcal{F} (f*\delta)(\xi)&=\mathcal{F} f(\xi)\mathcal{F} \mathds{1}(\xi)=\mathcal{F} f(\xi)\\
f(t)=(f*\delta)(t)&=\int_{-\infty}^{\infty}f(x)\delta(t-x)\,dx
}{
\mathcal{F} \mathds{1}(\omega)&=2\pi\delta(\omega)\\
\mathcal{F} (f*(2\pi\delta))(\omega)&=\mathcal{F} f(\omega)\mathcal{F} \mathds{1}(\omega)=\mathcal{F} f(\omega)\\
f(t)=\frac{1}{2\pi}(f*\delta)(t)&=\frac{1}{2\pi}\int_{-\infty}^{\infty}f(x)\delta(t-x)\,dx
}
