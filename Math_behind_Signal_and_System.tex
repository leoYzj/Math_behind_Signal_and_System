...
由于涉及分布级数及其换序问题，很难严格证明以上结果，我们将在附录\ref{sec:Schwartz_Functions}给出更一般的情况的证明，但它过于晦涩和复杂，或许还是直接接受这种形式上的运算更好一些。
...
因为我们总是可以用傅里叶变换的理论分析信号中各种（角）频率的成分。在电子信息领域，定义：
...
根据夹逼定理，$\|\mathcal{F} \varphi_k\|_{m,n}\to 0$，得证。
\end{proof}

我们曾在\ref{sec:Sampling and Interpolation}中，通过形式计算得到$\shah$分布的另一种定义：\begin{definition}[$\shah$的等价定义]
    \[\shah(t)=\sum_{k=-\infty}^{\infty}e^{2\pi ikt}\]
\end{definition}
现在可以根据傅里叶变换的连续性说明这种形式运算是正确的:\begin{proof}
设$\varphi\in\mathcal{S} $，定义分布$T=\sum_{k\in\mathbb{Z}}e^{2\pi ikt}$，则\begin{align*}
    \langle T,\varphi\rangle&=\langle \lim_{N\to\infty}\sum_{k=-N}^{N}e^{2\pi ikt},\varphi\rangle\\
   \langle \mathcal{F} T,\varphi\rangle&=\langle \mathcal{F} [\lim_{N\to\infty}\sum_{k=-N}^{N}e^{2\pi ikt}],\varphi\rangle\\
    &=\langle \lim_{N\to\infty}\mathcal{F} [\sum_{k=-N}^{N}e^{2\pi ikt}],\varphi\rangle\\
   &=\langle \lim_{N\to\infty}\sum_{k=-N}^{N}\delta_{2\pi k},\varphi\rangle\\
   &=\langle \shah_{2\pi},\varphi\rangle\\
   &=\langle \mathcal{F} \shah,\varphi\rangle
\end{align*}
因此$T=\shah$，得证。
注意\[\langle T,\varphi\rangle\neq \lim_{N\to\infty}\langle \sum_{k=-N}^{N}e^{2\pi ikt},\varphi\rangle\]
$\langle\rangle$表示在$\mathbb{R} $上积分，不能将它与极限进行交换。
\end{proof}
