...
\begin{proposition}[卷积的线性性、结合律、交换律]
    设$f,g_1,g_2\in L^1(\mathbb{R}),a,b\in \mathbb{C}$，则有
    \begin{align*}
...
\begin{proof}
设$f,g,h\in L^1(\mathbb{R})$，则\begin{align*}
...
\end{theorem}
\textbf{单连通}(single connected)是一个拓扑学概念，直观上讲，它表示$U$道路连通且“没有洞”，例如单位圆是单连通的，而圆环$\mathcal{A}= \{x+iy\in\mathbb{C}|1<x^2+y^2<2\}$不是单连通的。单连通的另一种等价表述是，单连通集合内的任意两条闭曲线都可以“连续地变化到彼此”，例如$\mathcal{A} $中$\gamma_1(t)=1.5e^{it},t\in[0,2\pi]$和$\gamma_2(t)=1.5+0.1e^{it},t\in[0,2\pi]$都是闭曲线，但它们不能连续地变化到彼此，因此$\mathcal{A} $不是单连通的。

根据Morera定理，单连通开集上的复线积分为0，则函数也是全纯函数，因此全纯函数与单连通开集上的复线积分为0是等价的。这种等价性表明可以用$\mathbb{R}^2$上的势场来理解全纯函数，其曲线积分具有路径无关性。对于了解微分形式的语言的读者，可以看出全纯函数$f=u+iv$相当于闭形式$u(x,y)\,dx+v(x,y)\,dy$（或者闭形式$-v(x,y)\,dx+u(x,y)\,dy$，对应另一个柯西-黎曼方程）。注意这种路径无关性只要求切换路径的区域上的全纯性，对于积分曲线包围不全纯点的情况，尽管复线积分一般不为0，但还是可以分段调整积分路径。例如对$f(z)=1/z$在曲线$\gamma(t)=e^{it},t\in[0,2\pi]$上积分，尽管$f$在包含原点的区域上不全纯，但可以将$\gamma$分成两段，分别在不包含原点的半圆上积分并调整积分路径。理解这一点将有利于理解后面的留数定理中对积分曲线的要求。

逻辑上，应该先有柯西积分公式，再有复变函数的幂级数展开，即\textbf{洛朗级数展开}(Laurant series expansion)，最后建立留数定理，但我们将直接引入洛朗级数，并借助洛朗级数建立对复线积分和留数定理的直观。

我们先补充一些复平面上的泰勒级数知识，再过渡到洛朗级数。
...
\end{definition}
这里$f$无限阶可导是由全纯性保证的，并且这些导数也全部是全纯函数。其实，单连通开集上的全纯函数一定是某个全纯函数的导数。 
...
\begin{theorem}[柯西-阿达马公式]
    ...
    在复幂级数的收敛域上，级数是绝对收敛且内闭一致收敛的。其本质还是与几何级数做比较。
\end{theorem}
这表明复幂级数的收敛半径与实幂级数的收敛半径求法一致，实幂级数的收敛半径可看作是复幂级数的收敛半径在实轴上的投影。前文中提到，复变函数的全纯性等价于解析性，因此全纯函数在其定义域内处处可展开成收敛于自身的泰勒级数。需要指出，在上述定义中我们其实已经给出一个很强的结论：只要$D(a,r)\in U$，则$f$在$a$处的泰勒级数在$D(a,r)$上收敛于$f$，即泰勒级数的收敛半径恰为$f$在$a$处到$U$的边界的距离，例如$f(z)=\frac{1}{1-z}$在$D(0,1)$上全纯，可以直接判断它在$z=0$处的泰勒级数$\sum_{n=0}^{\infty}z^n$的收敛半径为1，恰好是0到孤立奇点1的距离。

对于$f(z)=1/z$这样的函数，它已经是很简洁的分式形式了，但它在$z=0$处并不是全纯函数，因此不能在包含0的开集上展开成泰勒级数。我们将破坏了全纯性的这种点称为\textbf{孤立奇点}(isolated singular point)，孤立表示它不构成奇点集的极限点。为了处理这种情况，我们引入\textbf{洛朗级数}，它相比泰勒级数添加了负幂次项。

\begin{definition}[洛朗级数]
    设$a\in\mathbb{C}$，形如
    \[\sum_{n=-\infty}^{\infty}c_n(z-a)^n\]
    的级数称为以$a$为中心的\textbf{洛朗级数}。将它的负幂次项部分视为$w=1/(z-a)$下的幂级数，则可用柯西-阿达马公式确定它的收敛域为圆环$\mathcal{A} _a(r,R)$，其中$R,r$分别称为洛朗级数的\textbf{外半径}(outer radius)和\textbf{内半径}(inner radius)，满足：
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
将$f$展开为洛朗级数$f(z)=\sum_{n=-\infty}^{\infty}c_n z^n$，并在$\gamma$上积分，由于洛朗级数在闭曲线$\gamma$上内闭一致收敛，可以交换求和与积分的次序，得到
\begin{align*}
    \oint_{\gamma}f(z)\,dz&=\oint_{\gamma}\sum_{n=-\infty}^{\infty}c_n z^n\,dz\\
    &=\sum_{n=-\infty}^{\infty}c_n\oint_{\gamma}z^n\,dz\\
    &=2\pi i c_{-1}
\end{align*}
这就表明，洛朗级数中$(z-a)^{-1}$项的系数$c_{-1}$与复线积分有着密切的关系。尽管用它作为柯西积分定理的证明将导致循环论证，但它很好地解释了柯西积分公式的直观。
...
回到柯西积分公式，$\frac{f(\zeta)}{\zeta-z}$这一结构相当于为$f(\zeta)$添加了一个简单极点，于是包含这个零点的闭曲线上积分值恰好等于$2\pi i$倍的$f(z)$，这并不是一个偶然现象，实际上，复线积分的值与被积函数在闭曲线内部的孤立奇点系数有着密切的关系，这就是\textbf{留数定理}(residue theorem)：
\begin{theorem}[留数定理]
...
要找到$f$在$a$处的留数的计算公式，可以将$f$视同其在$a$处的洛朗级数展开式：
\[f(z)=\sum_{n=-k}^{\infty}c_n(z-a)^n\]
这里假设$a$为$f$的k阶极点。在处理泰勒级数时，我们在$f(x)=\sum_{n=0}^{\infty}\frac{f^{(n)}(a)}{n!}(x-a)^n$中两边同时对$x$求导再令$x=a$，得到$f^{(m)}(a)=m!c_m$。类似地，在处理洛朗级数时，我们可以将其乘以$(z-a)^k$从而化归到泰勒级数：
\begin{align*}
