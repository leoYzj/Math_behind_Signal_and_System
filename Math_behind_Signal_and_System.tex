...
我们总是用$z$表示复数$x+iy$。考虑复系数双变元多项式集$\mathbb{C}[x,y]$，通过做替换$x=\frac{z+\overline{z}}{2},y=\frac{z-\overline{z}}{2i}$，可以将$\mathbb{C}[x,y]$中的多项式$f(x,y)$表示为$g(z,\overline{z})$。在一些情况下，$g(z,\overline{z})$不显含$z$，例如$f(x,y)=x+iy$，$g(z,\overline{z})=z$。对于$f(x,y)$，我们可以用导数$\frac{\partial f}{\partial x},\frac{\partial f}{\partial y}$来描述它对$x,y$的依赖关系，同理，为了表述多项式对$z,\overline{z}$的依赖关系，我们定义:\begin{definition}[形式导数]
...
\begin{definition}[全纯函数]
    ...
    \begin{align*}
        \frac{\partial u}{\partial x}=\frac{\partial v}{\partial y},\frac{\partial v}{\partial x}=-\frac{\partial u}{\partial y},\text{或}\frac{\partial f}{\partial y}= i\frac{\partial f}{\partial x}
    \end{align*}
\end{definition}
...
全纯等价于极限\[f'(z)=\lim_{\zeta\to z}\frac{f(\zeta)-f(z)}{\zeta-z}\]
存在，即全纯等价于复可导性；全纯函数还是无限阶可导的，并且其各阶导数也是全纯函数，并且全纯性等价于解析性，即其泰勒级数在收敛域内一定收敛于这个函数本身：
...
以上定义其实隐含了一个很强的结论：只要$D(a,r)\in U$，则$f$在$a$处的泰勒级数在$D(a,r)$上收敛于$f$，即泰勒级数的收敛半径恰为$f$在$a$处到$U$的边界的距离，例如$f(z)=\frac{1}{1-z}$在$D(0,1)$上全纯，可以直接判断它在$z=0$处的泰勒级数$\sum_{n=0}^{\infty}z^n$的收敛半径为1，恰好是0到不全纯的点1的距离。
...
\begin{definition}[洛朗级数]
    ...
    \[R=\left(\varlimsup_{n\to\infty}|c_n|^{1/n}\right)^{-1},r=\left(\varlimsup_{n\to\infty}|c_{-n}|^{1/n}\right)^{-1}\]
...
\begin{definition}[拉普拉斯变换]
    ...
    其中$s$的实部$\sigma$必须满足$\sigma>a$，将它称为拉普拉斯变换的\textbf{收敛域}。\\
...
\end{definition}
在$\mathcal{E} $的定义中我们对$f$的定义域做了要求，但实际上只要$f$是因果信号（见\ref{sec:ODE}，$f(t)=f(t)u(t)$），就可以认为$f\in\mathcal{E}$，后文中不会特意区分$f$的定义域。
...
\begin{remark}
    其实，这个公式对实部大于$-1$的$\nu$都成立。尽管$-1<\text{Re}\ \ nu<0$时，$f(t)=t^\nu$在$t=0$处无界，但由于
...
这里$\mathcal{F} [f(t)e^{-\sigma t}](\omega)$实际上依赖于两个变量$\sigma,\omega$，但我们只写出$\omega$，并将$\sigma$视为一个参数。这样一来，我们就可以将拉普拉斯变换推广到$\delta$分布上：
\begin{align*}
    \langle \mathcal{L}\delta,\phi\rangle=\langle e^{-\sigma t}\delta,\mathcal{F} \phi\rangle=\mathcal{F}\varphi(0)=\int_{-\infty}^{\infty}\phi(t)\,dt=\langle \mathds{1},\phi\rangle
\end{align*}
其中$\mathds{1}$表示恒为1的函数。我们知道，多项式的拉普拉斯变换是真分式函数，而如果将$\delta$分布也纳入考虑，则可以得到任意有理函数。

用同样的方法，我们还可以得到$\delta$的平移的拉普拉斯变换：
\begin{align*}
    \langle \mathcal{L}\delta_a,\phi\rangle&=\langle e^{-\sigma t}\delta_a,\mathcal{F} \phi\rangle\\
    &=e^{-\sigma a}\mathcal{F}\varphi(a)\\
    &=\int_{-\infty}^{\infty}\phi(t)e^{-s a}\,dt\\
    &=\langle e^{-s a},\phi\rangle
\end{align*}
...
我们仍然令$g(t)=f(t)e^{-\sigma t}$，从而将傅里叶变换的逆变换公式推广到拉普拉斯变换上，得到拉普拉斯逆变换公式：
...
\begin{theorem}[*拉普拉斯反演公式]
    ...
    \[|F(s)|\leq C(1+|s|)^{-\alpha},\alpha>1,\forall \text{Re}\ s>a\]
    对$b>a,r>0$，定义时域上的函数族：
    \[f_{r,b}(t)=\frac{1}{2\pi i}\int_{b -ir}^{b +ir}F(s)e^{st}\,ds\]
    如果对某个$b>a$，当$r\to\infty$时，$f_{r,b}(t)\to f(t)\in\mathcal{E}$，则\[\forall b>a,\lim_{r\to\infty}f_{r,b}(t)=f(t),\text{且}F=\mathcal{L} f\]
\end{theorem}
\begin{remark}
    事实上，$\alpha>\frac{1}{2}$即可保证定理成立，但这样证明会非常复杂，这里不再讨论。
\end{remark}
\begin{proof}
    我们首先证明$\lim_{r\to\infty}f_{r,b}(t)$与$b$无关。设$b_1,b_2>a$，选择以$b_1\pm ir,b_2\pm ir$为顶点的矩形围道$C_r$，则由于$|F(s)|\leq C(1+|s|)^{-\alpha}$，矩形顶边、底边的积分趋于0，而$F(s)$在$\text{Re}\ s>a$上全纯，由柯西定理知$\int_{C_r}F(s)e^{st}\,ds=0$，因此$lim_{r\to\infty}f_{r,b_1}(t)=lim_{r\to\infty}f_{r,b_2}(t)=f(t)$。

    记$g_{b}(\omega)=F(b+i\omega),g_{r,b}(\omega)=\chi_{[-r,r]}(\omega)g_b(\omega)$，则
    \[g_b,g_{r,b}\in C(\mathbb{R});|F(s)|\leq C(1+|s|)^{-\alpha}\implies g_b,g_{r,b}\in L^1(\mathbb{R})\cap L^2(\mathbb{R} )\]
    并且$g_{r,b}$在$L^2$范数下收敛到$g_b$，由普朗歇尔定理（见\ref{sec:Fourier}），$\mathcal{F} g_{r,b}$在$L^2$范数下收敛到$\mathcal{F} g_b$，根据实变函数的理论，存在子列$\{\mathcal{F} g_{r_j,b}\}_{j=0}^{\infty}$，使得$\mathcal{F} g_{r_j,b}\to \mathcal{F} g_b,a.e.$。

    我们知道，\[f_{r,b}(t)=\frac{1}{2\pi}\int_{-r}^{r}F(b+i\omega)e^{(b+i\omega)t}\,d\omega=\frac{e^{b t}}{2\pi}\int_{-\infty}^{\infty}g_{r,b}(\omega)e^{i\omega t}\,d\omega=\frac{e^{bt}}{2\pi}\mathcal{F} g_{r,b}(-t)\]
    特别地，\[f_{r_j,b}(t)=\frac{e^{bt}}{2\pi}\mathcal{F} g_{r_j,b}(-t)\]
    令$j\to\infty$，有\[2\pi e^{-bt}f(t)=2\pi\lim_{j\to\infty}f_{r_j,b}(t)=\lim_{j\to\infty}\frac{e^{bt}}{2\pi}\mathcal{F} g_{r_j,b}(-t)=\mathcal{F} g_b(-t),a.e.\]
    因为$f \in\mathcal{E} $，我们可以选择充分大的$b$使得$e^{-bt}f(t)\in L^1(\mathbb{R} )$，由傅里叶反演公式（见附录\ref{sec:approach}的定理A.2.5.），
    \begin{align*}
        F(b+i\omega)&=g_b(\omega)\\
        &=\frac{1}{2\pi}\int_{-\infty}^{\infty}\mathcal{F} g_b(-x)e^{-i\omega x}\,dx\\
        &=\int_{0}^{\infty}e^{-bt}f(t)e^{-i\omega t}\,dt\\
        &=\mathcal{L} f(b+i\omega)
    \end{align*}
    即\[F=\mathcal{L} f,\forall \text{Re}\ s=b>a\]
\end{proof}
...
\noindent 法二：取极限法（又称为赫维赛德掩盖法）

按照以上命题给出的展开式形式设未知数，对于一次因式$(s-r)^m$，将等式两边同时乘以$(s-r)^m$，然后令$s\to r$，即可解出最高次幂的系数。对于次高次幂的系数，将等式两边同时乘以$(s-r)^{m-1}$，然后对$s$求导，再令$s\to r$，即可解出次高次幂的系数；也可以先约去已经求出的最高次幂项，再乘以$(s-r)^{m-1}$并令$s\to r$，依此类推。二次因式的处理类似，因为以上方法并不限于实数域。
...
在\ref{sec:freq_response}节中，我们通过傅里叶变换的卷积定理得到了系统函数，现在将通过拉普拉斯变换的卷积定理得到他：
\begin{definition}[系统函数]
    设$h\in\mathcal{E} $是线性时不变系统的冲激响应，则系统函数定义为$H(s)=\mathcal{L} h(s)$。
\end{definition}
需要指出，在本节中的系统函数都是基于拉普拉斯变换的，它与第四章中所使用的系统函数略有不同：它定义在复频域上，并且如果虚轴属于收敛域，则$H(i\omega)$与第四章中的系统函数一致。

与第四章的思路类似，我们可以通过系统函数来分析微分方程、动态电路，此外还可以通过系统函数来分析系统的稳定性，下面一一进行介绍。

拉普拉斯变换是处理常系数线性微分方程的有力工具。根据拉普拉斯变换的时域微分性质：
\[\mathcal{L} [f^{(n)}(t)](s)=s^n\mathcal{L} f(s)-s^{n-1}f(0+)-s^{n-2}f'(0+)-\cdots -f^{(n-1)}(0+)\]
可以将常系数线性微分方程转化为代数方程，还能够一并处理初始条件。我们先提出一个一般的解法，再通过一个具体的例子来展示拉普拉斯变换的威力。

考虑常系数线性微分方程
\[a_n r^{(n)}(t)+a_{n-1}r^{(n-1)}(t)+\cdots +a_1 r'(t)+a_0 r(t)=e(t)\]
其中$f,r\in\mathcal{E} $，并且给定初始条件
...
%先考虑一下要不要给拉普拉斯变换推广到右边[T,\infty)，这些问题处理起来很麻烦，这里应该还是先直接给非齐次项就好，带激励的导数的系统很麻烦，可以放到后边单独讨论
...
此外，还有一种基于实分析相关理论的傅里叶反演公式，我们仅仅给出其表述：
\begin{theorem}[傅里叶反演公式]
...
\begin{proof}
    由普朗歇尔定理，有
    \lr{
    \|\mathcal{F} f_k-\mathcal{F} f\|_2^2&=\int_{-\infty}^{\infty}|\mathcal{F} f_k(\xi)-\mathcal{F} f(\xi)|^2\,d\xi\\
    &=\int_{-\infty}^{\infty}|f_k(t)-f(t)|^2\,dt\\
    &=\|f_k-f\|_2^2
    }{
    \|\mathcal{F} f_k-\mathcal{F} f\|_2^2&=\frac{1}{2\pi}\int_{-\infty}^{\infty}|\mathcal{F} f_k(\omega)-\mathcal{F} f(\omega)|^2\,d\omega\\
    &=2\pi\int_{-\infty}^{\infty}|f_k(t)-f(t)|^2\,dt\\
    &=2\pi\|f_k-f\|_2^2
    }
    因此，$\|f_k-f\|_2\to 0\Rightarrow \|\mathcal{F} f_k-\mathcal{F} f\|_2\to 0$，得证。
\end{proof}
