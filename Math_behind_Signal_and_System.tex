...
\begin{lemma}
级数$\sum_{n=1}^{N}(f,\phi_n)\phi_n$依范数收敛，并且$\|\sum_{n=1}^{\infty}(f,\phi_n)\phi_n\|\leq\|f\|$
\end{lemma}
\begin{proof}
\begin{flalign*}
...
至此我们得到了傅里叶逆变换的定义：\begin{definition}[傅里叶逆变换]
...
\end{definition}
并由傅里叶级数展开式类比，从形式上得到了\textbf{傅里叶反演公式} (the Fourier Inversion Thoerem)：
\begin{claim}[傅里叶反演公式]
...
\begin{definition}[分布的傅里叶反变换]
    \begin{align*}
        \langle \mathcal{F} ^{-1}T,\varphi\rangle & =\langle T,\mathcal{F} ^{-1}\varphi\rangle,T\in\mathcal{T} ,\varphi\in\mathcal{S}
    \end{align*}
\end{definition}
...
\lr{
    \langle\mathcal{F}T^-,\varphi\rangle&=\langle T,\mathcal{F}\varphi^-\rangle\\
    &=\langle T,\mathcal{F} ^{-1}\varphi\rangle\\
    &=\langle\mathcal{F}^{-1}T,\varphi\rangle\\
    &\Rightarrow\mathcal{F}T^-= \mathcal{F}^{-1}T
}{
    \langle\mathcal{F}T^-,\varphi\rangle&=\langle T,\mathcal{F}\varphi^-\rangle\\
    &=\langle T,2\pi\mathcal{F} ^{-1}\varphi\rangle\\
    &=\langle2\pi\mathcal{F}^{-1}T,\varphi\rangle\\
...
设$f,\varphi\in\mathcal{S} ,T_f\in\mathcal{T} $，则
\begin{align*}
    \langle \overline{T_f},\varphi\rangle=\int_{-\infty}^{\infty}\overline{f(x)}\varphi(x)\,dx=\overline{\int_{-\infty}^{\infty}f(x)\overline{\varphi(x)}\,dx}=\overline{\langle T_f,\overline{\varphi}\rangle} \\
\end{align*}
...
因此我们定义\begin{definition}[分布与函数的卷积]
    \[\langle T*g,\varphi\rangle=\langle T,(g^-)*\varphi\rangle\]
\end{definition}
...
\begin{example}[$\delta_a$的平移作用]
    ...
    \begin{align*}
    \langle \delta_a*\delta_b,\varphi\rangle & =\langle \delta_a(y),\langle\delta_b(x),\varphi(x+y)\rangle\rangle \\
...
\begin{proof}
    ...
    \begin{align*}
    \langle\mathcal{F} T_n-\mathcal{F} T,\varphi\rangle & =\langle T_n-T,\mathcal{F} \varphi\rangle \\
    \lim_{n\to\infty}\langle\mathcal{F} T_n-\mathcal{F} T,\varphi\rangle & =\lim_{n\to\infty}\langle T_n-T,\mathcal{F} \varphi\rangle=0
\end{align*}
\end{proof}
...
\end{definition}
容易看出，这个分布具有两条基本性质，其一是前面提到的周期化：$\shah_T*f(t)=f_T(t)$，其二是取样性质，它直接得自$\delta_a$的
...
\begin{theorem}[采样函数的“卷积幺元”作用]
    设带限函数$f(t)$满足$supp\mathcal{F} f\subset [-T/2,T/2]$，则\lr{
    f(t)=Tsinc(Tt)*f(t)
}{
...
证明中，第二行到第三行的积分与求和的换序用到了周期化函数作为函数项级数的一致收敛性。这个等式意味着对时
...
式子。需要注意，这个式子仅在分布的意义下成立，例如将这个正则分布作用在某一区间的特征函数$\chi_{[a,b]}\in\mathcal{S}$上，则可验证函数项级数$\sum_{k=-\infty}^{\infty}e^{-2\pi ikx}$在整数点处趋于正无穷，在任意不包含整数点的区间上积分值为0。然而，作为函数项级数，我们只能得到整数点处的广义极限存在，这是一个几乎处处不收敛的函数项级数，例如在$x=1/2$处，级数项为$(-1)^k$。
...
完全类似的恒等式，称为\textbf{抽样定理}(the sampling thoerem)：
\begin{theorem}[抽样定理]
...
行无限多次抽样，亦不可能做到等间距抽样，解决这些问题的一种方式是做近似，另一种方式是有限采样，将在下一节讨论。

我们来明确一下取样与插值要讨论的问题：\begin{definition}[取样与插值问题]
设有确定信号$f(t)$，通过取样的方式找出它在若干点处的\textbf{样值}$f(t_0),f(t_1),f(t_2),\cdots$，在怎样的情况下可以借助这组值还原出$f(t)$，即在离散的数据点之间应如何“插入”新的数据点使之成为一条连续的曲线（这也是名称“插值”的由来）。一般地，相邻取样点的间隔一定，记为$T_s$，称为\textbf{抽样间隔}，将$f_s=1/T_s$称为\textbf{抽样频率},将$\omega_s=2\pi f_s=2\pi/T_s$称为\textbf{抽样角频率}。
\end{definition}
类似于用像素表示图片，利用抽样所得的离散信号$f_d[n]=f(nT_s)$表示信息，相当于用离散信号表示连
...
一点的值，换言之，在以下这种公式所代表的插值方式下，数据点处的值是不会产生偏移
的，$Sa$函数同理。
\[f(t)=\sum_{k=-\infty}^{\infty}f\left(\frac{k}{T}\right)sinc\left(T\left(t-\frac{k}{T}\right)\right)\]
...
\begin{definition}[零输入响应与零状态响应]
    由于系统原有的能量，初始状态可能不为零，这时即便激励$e(t)=0$，响应$r(t)$也可能不为零。根据系统的线性性，可以将r(t)分解为两部分：\textbf{零输入响应}(zero input response,$r_{z.i}(t)$)和\textbf{零状态响应}(zero state response,$r_{z.s}(t)$)，零输入响应是指$e(t)=0$，由初始状态产生的响应，零状态响应则是将初始状态置零时的响应。\textbf{全响应}$r(t)=r_{z.i}(t)+r_{z.s}(t)$。
\end{definition}
...
\end{definition}
从研究微分方程描述系统的线性、时不变性的过程可以看出，提出零状态响应、零输入响应的分解是必要的，并且我们还将很快看到这样的分解方式对于求解常系数线性微分方程的好处。
...
\begin{definition}[因果信号]
    以上例子中出现了满足$h(t)=y_h(t)u(t)$的信号，将这种信号称为\textbf{因果信号}(causal signal)
\end{definition}

我们还可以对以上结果做一些推广：求解微分方程
\[\sum_{n=0}^{N}a_n r^{(n)}(t)=\sum_{m=0}^{M}b_m \delta^{(m)}\]
当$r(t)$的最高次导的阶数大于$\delta$的最高阶导阶数，即$N> M$时，解为
$y_h(t)u(t)$；当$r(t)$的最高次导的阶数不小于$\delta$的最高阶导阶数，即$N\leq M$时，
...
\begin{example}
    ...
    带入原方程得到：\begin{align*}
        LHS=&(r_h''(t)u(t)+r_h'(0)\delta+r_h(0)\delta'+C\delta'')\\
        &+3(r_h'(t)u(t)+r_h(0)\delta+C\delta')\\
        &+2(r_h(t)u(t)+C\delta)\\
        =&(r''_h(t)+3r'_h(t)+2r_h(t))u(t)+(r'_h(0)+3r_h(0)+2C)\delta+(r_h(0)+3C)\delta'+C\delta''
    \end{align*}
    对比系数，得到：\begin{align*}
        \begin{cases}
            r'_h(0)+3r_h(0)+2C=1\\
        ...
        \end{cases}
    \end{align*}
    所以$C=1,r_h(0)=-3,r'_h(0)=8$，注意$u(t)$的系数自然为0，这是由齐次解的性质保证的。满足上述条件的$C_1=2,C_2=-5$，因此单位脉冲响应为$h(t)=(2e^{-t}-5e^{-2t})u(t)+\delta$。
\end{example}
...
\begin{definition}[系统函数]
    将$H(\omega)$称为$r(t)$的\textbf{系统函数}。
\end{definition}
一个物理可实现的系统，容易理解它必须满足因果性
...
，我们知道线性的电感、电容在相量法下可以与线性电阻等同处理，统一地用\textbf{阻抗}来
...
我们曾在\ref{sec:convolution}中提到，滤波就是把信号中的某些成分剔除或大幅减少，相当于在频域乘以一个函数，在时域上的表现则是与这个函数的傅里叶逆变换进行卷积。从系统函数的视角来看，滤波器就是一个系统，理想滤波器的系统函数在某些频率处接近于0，而在另一些频率处接近于1。
\begin{definition}[滤波器的分类]
    低通滤波器是指系统函数$H(\omega)$在低频段（$|\omega|<\omega_c$，$\omega_c$为截止频率）接近于1，而在高频段（$|\omega|>\omega_c$）接近于0的滤波器；高通滤波器则相反，在高频段接近于1，在低频段接近于0；带通滤波器是指系统函数在某一频率区间内接近于1，而在该区间外接近于0的滤波器；带阻滤波器则相反，在某一频率区间内接近于0，而在该区间外接近于1的滤波器。
\end{definition}
\begin{figure}[H]
    \centering
...
将$H(\omega)$看作一个复数，即$H(\omega)=|H(\omega)|e^{i\varphi(\omega)}$，则$|H(\omega)|$表示系统对频率为$\omega$的信号成分的放大倍数，$\varphi(\omega)$表示该频率成分经过系统后相位的变化。我们定义:
\begin{definition}[幅频特性和相频特性]
    $|H(\omega)|-\omega$称为系统的\textbf{幅频特性}，$\varphi(\omega)-\omega$
称为系统的\textbf{相频特性}，这里$\varphi(\omega)$通常取主值，即$(-\pi,\pi]$内的值。
\end{definition}
一般而言，理想滤波器的单位脉冲响应是实函数，从而系统函数满足$H(\omega)=\overline{H(-\omega)}$，即幅频特性是偶函数，相频特性是奇函数。
\begin{example}
将激励信号$V_{in}(t)=\cos(\omega_0 t)$施加于例10中的RC电路，求输出
信号$V_{out}(t)$\\
解：激励信号的傅里叶变换为
...
\begin{align*}
    ...
    &=Re\left[\frac{1}{1+i\omega_0 RC}e^{i\omega_0 t}\right]\\
...
\begin{align*}
    ...
    &r(t)=|H(\omega_0)|\cos(\omega_0 t+\varphi(\omega_0))
\end{align*}
...
将以上过程进一步抽象:
\begin{align*}
    \mathcal{F}[H[e^{i\omega_0 t}]]&=\mathcal{F} [h(t)*e^{i\omega_0 t}]\\
    &=H(\omega)\mathcal{F} [e^{i\omega_0 t}]\\
    &=2\pi H(\omega)\delta_{\omega_0}\\
    &=2\pi H(\omega_0)\delta_{\omega_0}
\end{align*}
因此\[r(t)=H[e^{i\omega_0 t}]=H(\omega_0)e^{i\omega_0 t}\]
\begin{definition}[系统的频率响应特性]
    将函数$H(\omega)=H[e^{i\omega t}]$称为系统的\textbf{频率响应特性}。给定$\omega$，将函数$e^{i\omega t}$称为系统H的\textbf{特征函数}(eigenfunction)，\textbf{特征值}(eigenvalue)为$H(\omega)$。
\end{definition}

频率响应特性从数学上重复了系统函数的定义，但研究$H[e^{i\omega t}]$是很有意义的，我们将在\ref{sec:DFT}中看到，这能够将表示离散线性时不变系统的循环矩阵对角化，其过渡矩阵是一种特殊的范德蒙德矩阵，要直接在高等代数中获得这个结论是很困难的。
...
的统一延迟，即\[r(t)=Ke(t-t_0)\]其中K为常数，$t_0$为常数延迟。由傅里叶变换的时移性质可知，无失真传输系统的系统函数为
\[H(\omega) = Ke^{-i\omega t_0}\]可以看到，$|H(\omega)|=K,t_0=-\varphi(\omega)/\omega$，其幅频特性是常数，图像为一水平线；相频特性是线性函数，图像为过原点的直线，斜率$-t_0$对应系统造成的延时。

实际上，由于物理限制，我们不可能实现一个无失真传输系统，但只要所讨论的信号的频谱基本都集中在特定的频段当中，我们就可以近似认为在这个频段上满足$|H(\omega)|=K,\varphi(\omega)=-\omega t_0$的系统是无失真传输系统。
...
对于无失真传输系统，$\tau_p(\omega)=\tau_g(\omega)=t_0$。
根据电路分析的经验，看到$\tau_p(\omega)=-\varphi(\omega)/\omega$而定义$\tau_g(\omega)=-d\varphi(\omega)/d\omega$
是自然的，在处理非线性电阻的伏安特性曲线时也曾类似地定义动态电阻和静态电阻，但其
物理意义并不十分明显，我们通过两个例子来理解它们的意义。

\begin{example}[相时延]
设输入信号为单频复指数信号$e^{i\omega_0 t}$，求系统函数为$H(\omega)$
的线性时不变系统的输出信号$r(t)$。\\
解：我们已经在前面得到：\[r(t)=H[e^{i\omega_0 t}]=H(\omega_0)e^{i\omega_0 t}\]
...
其中$\omega_c$为载波角频率，$\omega_m$为被调制信号的角频率，且$\omega_c\gg \omega_m$.系统H的单位脉冲响应$h(t)$为实信号，系统函数为$H(\omega)$。求$e(t)$进入系统所得的输出信号$r(t)$。\\
解：
...
\begin{align*}
    ...
    =&\frac{\pi}{2} [H(\omega_c+\omega_m)\delta_{\omega_c+\omega_m}+\overline{H(\omega_c+\omega_m)}\delta_{-(\omega_c+\omega_m)}\\
    &+H(\omega_c-\omega_m)\delta_{\omega_c-\omega_m}+\overline{H(\omega_c-\omega_m)}\delta_{-(\omega_c-\omega_m)}]
\end{align*}
反变换得到输出信号为
\begin{align*}
    r(t)=&\frac{1}{4}[H(\omega_c+\omega_m)e^{i(\omega_c+\omega_m)t}+\overline{H(\omega_c+\omega_m)}e^{-i(\omega_c+\omega_m)t}\\
    &+H(\omega_c-\omega_m)e^{i(\omega_c-\omega_m)t}+\overline{H(\omega_c-\omega_m)}e^{-i(\omega_c-\omega_m)t}]\\
    =&\frac{1}{2}Re\left[H(\omega_c+\omega_m)e^{i(\omega_c+\omega_m)t}+H(\omega_c-\omega_m)e^{i(\omega_c-\omega_m)t}\right]\\
...
这里波数并不是指波的数目，而是一个将空间距离折算为相位延迟的比例系数。我们将宗量$\omega t-kx$称为波的\textbf{相位}(phase)，考察相位的传播速度，即令$\omega t-kx=const$，得到
...
因此可以对给定的$\omega$求出系统函数为
\[H(\omega)=e^{-i\omega \frac{L}{u}}=e^{-ikL},k=\frac{\omega}{u}\]
可以看出，$k$实际上是关于$\omega$的函数$k(\omega)=\omega/u$，但在研究单频正弦波时看不出这一点。

一般地，设初始波形为$f(x)$，波速为$u$，可以确定$x=0$处的激励信号$e(t)=f(-ut)$。它的不同频率成分通过均匀色散介质构成的系统时，只产生相位失真，因此系统函数可以表示为
...
\begin{definition}
    波包即脉冲波形，\textbf{波包中心}是指脉冲波形中振幅最大的位置，其传播速度称为\textbf{波包速度}。
\end{definition}
对于窄带光脉冲，群速度等于波包速度，因此也是能量和信息在介质中传播的速度。
...
\[V_{DSB}(\omega)=\frac{\pi}{2}[\delta_{\omega_c+\omega_m}+\delta_{-(\omega_c+\omega_m)}+\delta_{\omega_c-\omega_m}+\delta_{-(\omega_c-\omega_m)}]\]
...
事实上，对于带限信号（尤其是窄带信号），我们都可以采用这种调制解调的方法进行传输，这是由于三角函数具有\textbf{频谱搬移}(spectrum shifting)的性质：\begin{align*}
    \mathcal{F} [\cos(\omega_c t)]&=\pi[\delta_{\omega_c}+\delta_{-\omega_c}]\\
    \mathcal{F}[x(t)\cos(\omega_c t)]&=\frac{1}{2\pi}X(\omega)*\mathcal{F}[\cos(\omega_c t)]\\
...
本小节将串写拉普拉斯变换所需的复变函数论中的一些基本概念和重要结果。实际上，只需要复变函数中一些最基础的概念即可建立起拉普拉斯变换理论的大部分内容，较为困难的部分在于用留数定理计算拉普拉斯逆变换，但这不会影响多数情况下的拉普拉斯逆变换的求解。因此，即便是未学过复变函数的读者，也可通过本小节快速把握拉普拉斯变换中所需的复变函数的内容。

本小节我们主要参考美国数学会的经典教材Function Thoery of One Complex Variable by Robert E. Greene and Steven G. Krantz。后文中所述的逻辑关系与证明均可在该书中找到。在其他教材中，对逻辑关系可能有不同的的处理方式，特此说明。

我们总是用$z$表示复数$x+iy$，$i$为虚数单位$\sqrt{-1}$。考虑复系数双变元多项式集$\mathbb{C}[x,y]$，通过做替换$x=\frac{z+\overline{z}}{2},y=\frac{z-\overline{z}}{2i}$，可以将$\mathbb{C}[x,y]$中的多项式$f(x,y)$表示为$g(z,\overline{z})$。在一些情况下，$g(z,\overline{z})$不显含$z$，例如$f(x,y)=x+iy$，$g(z,\overline{z})=z$。对于$f(x,y)$，我们可以用导数$\frac{\partial f}{\partial x},\frac{\partial f}{\partial y}$来描述它对$x,y$的依赖关系，同理为了表述多项式对$z,\overline{z}$的依赖关系，我们定义:\begin{definition}[形式导数]
    设$f=u+iv\in C^1(\mathbb{C})$，$u,v$为复数域上的实值函数，则
    \begin{align*}
        \frac{\partial f}{\partial z}=\frac{1}{2}(\frac{\partial}{\partial x}-i\frac{\partial}{\partial y})f=\frac{1}{2}(\frac{\partial u}{\partial x}+\frac{\partial v}{\partial y})+\frac{i}{2}(\frac{\partial v}{\partial x}-\frac{\partial u}{\partial y})\\
        \frac{\partial f}{\partial \overline{z}}=\frac{1}{2}(\frac{\partial}{\partial x}+i\frac{\partial}{\partial y})f=\frac{1}{2}(\frac{\partial u}{\partial x}-\frac{\partial v}{\partial y})-\frac{i}{2}(\frac{\partial v}{\partial x}+\frac{\partial u}{\partial y})
    \end{align*}
\end{definition}
...
容易想到对一般的复变函数（而不只是多项式）也可以采用以上形式导数的定义。这种形式导数在其他方面也很像普通的导数，甚至可以说$z,\overline{z}$构成了一组新的坐标。例如，可以验证形式导数的运算仍然满足莱布尼兹法则和链式法则：
\begin{claim}[莱布尼兹法则]
...
\begin{claim}[链式法则]
    ...
    \end{align*}
\end{claim}
以上结论的证明略去，它们只是多元微分学的小练习。下面提出复变函数中最重要的\textbf{全纯函数}(holomorphic function)的概念：
...
有很多种方法来理解它，例如，从显然的同构$\mathbb{C}\cong \mathbb{R}^2$出发，将$f:\mathbb{C}\to\mathbb{C}$看作$f:\mathbb{R}^2\to\mathbb{R}^2$，则它的导数构成$\mathbb{R}-$线性映射；而如果$f$是全纯函数，它的导数构成$\mathbb{C}-$线性映射；全纯等价于极限\[f'(z)=\lim_{\zeta\to z}\frac{f(\zeta)-f(z)}{\zeta-z}\]
存在，即全纯等价于复可导性；作为$f:\mathbb{R}^2\to\mathbb{R}^2$，其雅可比矩阵
\[J_f(z)=\begin{bmatrix}\frac{\partial u}{\partial x}&\frac{\partial u}{\partial y}\\ \frac{\partial v}{\partial x}&\frac{\partial v}{\partial y}\end{bmatrix}\]
在柯西-黎曼方程下化为
\[J_f(z)=\begin{bmatrix}a&-b\\ b&a\end{bmatrix}=\begin{bmatrix}\lambda&0\\0&\lambda\end{bmatrix}\begin{bmatrix}\cos\theta&-\sin\theta\\ \sin\theta&\cos\theta\end{bmatrix}\]
其中$a=\frac{\partial u}{\partial x}=\frac{\partial v}{\partial y},b=\frac{\partial v}{\partial x}=-\frac{\partial u}{\partial y}$，$\lambda=a^2+b^2$，$\theta=\arctan(b/a)$。这表明全纯函数在局部上具有共形性质，经$f$映射，$z$一个充分小的增量被乘以$\lambda e^{i\theta}$，即幅值变为$\lambda$倍，辐角旋转$\theta$；此外，全纯性等价于解析性，即无限阶可导的函数，其泰勒级数在收敛域内一定收敛于这个函数本身，对实变函数这未必成立：
\begin{example}
    设$f(x)=\begin{cases}
    e^{-1/x}&\text{,if\ }x> 0\\
    0&\text{,if\ }x\leq 0
...
\end{example}
另外，如果建立同构\[\mathbb{C}\cong \{xI+yJ\big| (x,y)\in\mathbb{R}^2\},I=\begin{bmatrix}
    1&0\\
    0&1
\end{bmatrix},J=\begin{bmatrix}
    ...
    1&0
\end{bmatrix}\]
则全纯函数的雅可比矩阵是同构意义下的复数，即“复可导函数的导数是复数”。
...
\end{theorem}
\textbf{单连通}(single connected)是一个拓扑学概念，直观上讲，它表示$U$道路连通且“没有洞”，例如单位圆是单连通的，而圆环$\mathcal{A}_0(1,2)$不是单连通的。单连通的另一种等价表述是，单连通集合内的任意两条闭曲线都可以“连续地变化到彼此”，例如$\mathcal{A} $中$\gamma_1(t)=1.5e^{it},t\in[0,2\pi]$和$\gamma_2(t)=1.5+0.1e^{it},t\in[0,2\pi]$都是闭曲线，但它们不能连续地变化到彼此，因此$\mathcal{A} $不是单连通的。但来南通的一种等价表述是，单连通集合内的任意闭曲线都可以连续地变化到一个点，例如单位圆内的任意闭曲线都可以连续地变化到圆心。
...
其实，全纯函数还是无限阶可导的，并且其各阶导数也是全纯函数，我们在前面断言“全纯$\Leftrightarrow$解析”已经隐含了这个结论。下面给出复变函数的泰勒级数展开式：
...
\end{definition}
这个定义其实隐含了一个很强的结论：只要$D(a,r)\in U$，则$f$在$a$处的泰勒级数在$D(a,r)$上收敛于$f$，即泰勒级数的收敛半径恰为$f$在$a$处到$U$的边界的距离，例如$f(z)=\frac{1}{1-z}$在$D(0,1)$上全纯，可以直接判断它在$z=0$处的泰勒级数$\sum_{n=0}^{\infty}z^n$的收敛半径为1，恰好是0到不全纯的点1的距离。

数学分析中，我们学过幂级数收敛半径的求法，复变函数的泰勒级数也有类似的结论：
\begin{theorem}[柯西-阿达马公式]
    复幂级数\[\sum_{n=0}^{\infty}c_n(z-a)^n\]
    的收敛域为$D(a,R)$，其中收敛半径$R$由下式给出：
    \[R=(\varlimsup_{n\to\infty}|c_n|^{1/n})^{-1}\]
    这里的极限是广义极限，即允许$R=0$（仅在中心点收敛）或$R=\infty$（级数在整个复平面上收敛）。
\end{theorem}
在复幂级数的收敛域上，级数是绝对收敛且内闭一致收敛的。其本质还是与几何级数做比较，收敛半径与实幂级数的收敛半径求法一致，实幂级数的收敛半径可看作是复幂级数的收敛半径在实轴上的投影。使用柯西-阿达马公式求出的收敛半径与前面提到的“到不全纯区域的距离”总是一致的。
...
\begin{definition}[洛朗级数]
    ...
    \[R=(\varlimsup_{n\to\infty}|c_n|^{1/n})^{-1},r=(\varlimsup_{n\to\infty}|c_{-n}|^{1/n})^{-1}\]
    在圆环$\mathcal{A} _a(r,R)$上，洛朗级数是绝对收敛且内闭一致收敛的。
\end{definition}
...
\begin{align*}
    ...
    &=\begin{cases}
        ...
        i\left[\frac{e^{i(1-n)t}}{i(1-n)}\right]_0^{2\pi}=0&n\neq 1
    \end{cases}
\end{align*}
...
\end{align*}
尽管用它作为柯西积分定理的证明将导致循环论证，但它很好地解释了柯西积分公式的直观。
...
\begin{definition}[孤立奇点的分类]
    ...
    \begin{enumerate}
        ...
        \item 如果存在$m\in\mathbb{Z}^+$使得$c_{-1}=c_{-2}=\cdots=c_{-m}=0,c_{-(m+1)}\neq 0$，则称$a$为$f$的\textbf{m阶极点}(pole of order m)；
        其中$m=1$时，称$a$为$f$的\textbf{简单极点}(simple pole)；
        \item 如果$c_n\neq 0$对无穷多个负整数$n$成立，则称$a$为$f$的\textbf{本性奇点}(essential singularity)。
    \end{enumerate}
\end{definition}
例如，$f(z)=e^{1/z}$在0处有本性奇点，$f(z)=1/\sin(\pi z)$在$z=n,n\in\mathbb{Z}$处有简单极点，$f(z)=\frac{\sin z}{z}$在0处有可去奇点。$k$阶极点意味着$f(z)$在$a$处趋于无穷的速度与$1/(z-a)^k$同阶。
...
\begin{theorem}[留数定理]
    ...
    \[\oint_{\gamma}f(z)\,dz=2\pi i\sum_{k=1}^{n}\text{Res}(f,a_k)Ind_{\gamma}(a_k)\]
    其中$\text{Res}(f,a_k)$称为$f$在$a_k$处的\textbf{留数}(residue)，它等于$f$在$a_k$处洛朗级数展开式中$(z-a_k)^{-1}$项的系数；$Ind_{\gamma}(a_k)$称为$\gamma$绕点$a_k$的\textbf{环绕数}(winding number)，它表示$\gamma$绕$a_k$转了多少圈。
\end{theorem}
...
\begin{align*}
    ...
    \frac{d^(k-1)}{dz^(k-1)}((z-a)^k f(z))&=\sum_{n=0}^{\infty}c_{n-k}\frac{d^{(k-1)}}{dz^{(k-1)}}(z-a)^n\\
    &=\sum_{n=k-1}^{\infty}c_{n-k}\frac{n!}{(n-k+1)!}(z-a)^{n-k+1}
\end{align*}
...
\begin{example}
    ...
    解：根据定义，$\mathcal{F} f(\omega)=\int_{-\infty}^{\infty}\frac{1}{a^2+t^2}e^{-i\omega t}\,dt$。为了使用留数定理计算该积分，我们将其推广为复变量上的积分，注意在$\mathbb{R}$上$z=Re\,z=t$：
...
\section{拉普拉斯变换初步}\label{sec:Laplace_Transform}
...
鉴于拉普拉斯变换的应用场景中使用角频率明显多于使用频率，我们在本书中采用角频率$\omega$作为拉普拉斯变换的变量。
...
\begin{definition}[拉普拉斯变换]
    记\[\mathcal{E}=\{f:[0,\infty)\to\mathbb{C}|f\in PC[0,\infty),\exists C>0,a\in\mathbb{R},|f(t)|<Ce^{at},\forall t\geq 0\}\]
    其中$PC[0,\infty)$表示$[0,\infty)$上的分段连续函数。
    对$f\in\mathcal{E}$，其拉普拉斯变换定义为
    \[\mathcal{L} f(s)=\int_{0}^{\infty}f(t)e^{-st}\,dt,\quad s=\sigma+i\omega\]
    其中$s$的实部$\sigma$必须满足$\sigma>a$。\\
    如果$f(t)$是时域信号，$\mathcal{L} f(s)$称为\textbf{复频域信号}。
    我们仍记$\mathcal{L} f(s)$为$F(s)$或$\hat{f}(s)$，并记$f\overset{\mathcal{L} }{\longleftrightarrow} F(s)$。
\end{definition}
在$\mathcal{E} $的定义中我们对$f$的定义域做了要求，实际上只要$f$是因果信号（见\ref{sec:ODE}，$f(t)=f(t)u(t)$），就可以认为$f\in\mathcal{E}$，后文中不会特意区分$f$的定义域。

在$\mathcal{E} $上定义拉普拉斯变换，是因为对于$f\in\mathcal{E} $，当$\sigma>a$时，拉普拉斯变换的积分是绝对收敛的：
\begin{align*}
    |\mathcal{L} f(s)|&=\left|\int_0^{\infty}f(t)e^{-st}\,dt\right|\\
    &\leq \int_0^{\infty}|f(t)||e^{-st}|\,dt\\
    &\leq \int_0^{\infty}Ce^{at}e^{-\sigma t}\,dt\\
    &=C\int_0^{\infty}e^{-(\sigma-a)t}\,dt=\frac{C}{\sigma-a}<\infty
\end{align*}

在$\sigma>a$时，$\mathcal{L} f(s)$关于$s$是全纯函数，可以对$s$求导：
\begin{align*}
    ...
    &=-\mathcal{L} \{tf(t)\}(s)
\end{align*}
根据全纯函数的唯一延拓性，如果$\mathcal{L} f(s)$可以延拓至$\sigma\leq a$的一些区域，则延拓是唯一的，我们也认为延拓所得的定义域更大的函数是$f$的拉普拉斯变换。
\begin{example}[多项式的拉普拉斯变换]
    ...
    \begin{align*}
        ...
        &=\frac{\Gamma(\nu+1)}{s^{\nu+1}},\quad \sigma>0
    \end{align*}
尽管积分只在$\sigma>0$时收敛，但$\mathcal{L} f(s)$可以延拓为$\mathbb{C}\backslash\{0\}$上的全纯函数，定义域为$\mathbb{C}\setminus\{0\}$。
\end{example}

在$\mathcal{E} $中，卷积总是有定义的：\begin{definition}[$\mathcal{E} $中的卷积]
    设$f,g\in\mathcal{E} $，则它们的卷积定义为
    \[(f*g)(t)=\begin{cases}
    0                   & \text{if } t<0 \\
    \int_0^t f(x)g(t-x)\,dx & \text{if } t\geq 0
    \end{cases}\]
\end{definition}
我们并没有改变卷积的定义，只是在被积函数的自变量非负，即$x\geq 0,t-x\geq 0$时，$f(x),g(t-x)$才不恒为0，这相当于积分区间为$[0,t]$。
...
\begin{proof}
    ...
    \end{align*}
    3.频移：\begin{align*}
        \mathcal{L} \{e^{a t}f(t)\}(s)&=\int_0^{\infty}e^{a t}f(t)e^{-st}\,dt\\
    ...
    5.时域微分：\begin{align*}
        ...
        &=\evalat{(f(t)e^{-st})}{0}{\infty}+\int_0^{\infty}sf(t)e^{-st}\,dt\\
        &=s\mathcal{L} f(s)-f(0+)
    \end{align*}
    6.复频域微分：已经在本节前面证明过。\\
    7.时域积分：\begin{align*}
        \mathcal{L} \left\{\int_0^t f(x)\,dx\right\}(s)&=\int_0^{\infty}\left(\int_0^t f(x)\,dx\right)e^{-st}\,dt\\
        &=\left[\frac{-1}{s}e^{-st}\int_0^t f(x)\,dx\right]_0^{\infty}+\frac{1}{s}\int_0^{\infty}f(t)e^{-st}\,dt\\
...
以上所有性质的证明都与傅里叶变换类似，实际上我们不需要做这种重复性的证明工作。给定$s=\sigma+i\omega$，令$\tilde{f}(t)=f(t)e^{-\sigma t}$，则$\mathcal{L} f(s)=\mathcal{F} \tilde{f}(\omega)$，只要$\tilde{f}\in L^1(\mathbb{R} )$，就可以将傅里叶变换的性质直接应用到拉普拉斯变换上。由于我们在$\mathcal{E} $中给出了$f(t)$的分段连续性，在收敛域$\sigma>a$内，$\tilde{f}(t)$总是属于$L^1(\mathbb{R} )$的。下面给出两个例子：
\begin{example}[时移性质]
    设$f\in\mathcal{E} ,t_0>0$，令$\tilde{f}(t)=f(t)e^{-\sigma t}$，则
    \begin{align*}
        \mathcal{L} \{f(t-t_0)u(t-t_0)\}(s)&=\mathcal{F} \{\tilde{f}(t-t_0)u(t-t_0)\}(\omega)\\
...
有
    \begin{align*}
        \mathcal{L} \{(f*g)(t)\}(s)&=\mathcal{F} [f*g(t)e^{-\sigma t}](\omega)\\
        &=\mathcal{F} [\tilde{f}*\tilde{g}](\omega)\\
...
用同样的方法，可以将傅里叶变换的逆变换公式推广到拉普拉斯变换上，得到拉普拉斯逆变换公式：
\begin{theorem}[拉普拉斯逆变换]
    设$f\in\mathcal{E} $，则
...
\begin{remark}
    ...
    应该理解为沿着复平面上的直线$Re z=b$从$b-i\infty$到$b+i\infty$的积分。由于全纯函数的路径无关性，我们也可以选择其他路径，甚至可以添加一些路径，再使用留数定理计算拉普拉斯逆变换，见下一节的讨论。
\end{remark}
\begin{proof}
    \begin{align*}
...
\begin{example}[指数多项式]
    设$f(t)=t^n e^{a t}u(t),n\in\mathbb{N} $，利用拉普拉斯变换的频移性质，有
    \begin{align*}
        \mathcal{L} f(s)&=\mathcal{L} \{t^n e^{a t}u(t)\}(s)\\
        &=\mathcal{L} \{t^n u(t)\}(s-a)\\
        &=\frac{\Gamma(n+1)}{(s-a)^{n+1}},\quad \sigma>a
    \end{align*}
...
最后我们来介绍一种拉普拉斯变换的级数求法。我们知道，多项式的拉普拉斯变换都可以通过$\mathcal{L} [t^n](s)=\frac{\Gamma(n+1)}{s^{n+1}}$来计算，那么对于一些可以展开为幂级数的函数，不难想到逐项求拉普拉斯变换，下面讨论这种操作的适用范围。在此之前，我们先介绍一个引理：
\begin{lemma}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\implies \forall s,Re s=\omega_0+\phi>\omega_0,|f(t)e^{-s t}|\leq Ce^{-\phi t}
    \end{align*}
    因此控制收敛定理的条件成立，换序即证。
\end{proof}
...
\section{拉普拉斯变换进阶}\label{sec:Advanced_Laplace}
...
$f\in PS([0,T])\Longleftrightarrow$除有限个点外f均可导，并且这些点是f的第一类间断点
\end{definition}
...
\begin{proof}
\begin{align*}
    S_N^f(t_0)-\frac{f(t_0^+)+f(t_0^-)}{2} =&\frac{1}{T}\left(\int_{T}f(t_0-\tau)D_N(\tau)\,d\tau-\int_{0}^{\frac{T}{2}}f(t_0^+)D_N(\tau)\,d\tau\right.\\
    &\left.-\int_{-\frac{T}{2}}^{0}f(t_0^-)D_N(\tau)\,d\tau\right) \\
...
在\ref{sec:Sampling and Interpolation}中，我们对带限函数提出了公式：如果$supp\mathcal{F} f\subset[-\frac{T}{2},\frac{T}{2}]$，则有
\lr{
    f(t)=Tsinc(Tt)*f(t)
}{
    f(t)=\frac{T}{2\pi}Sa(Tt)*f(t)
...
如果希望对一般的函数，找到一个函数$g(x)$，使得$f(x)=g*f(x)$，自然会想到狄拉克$\delta$分布，然而它是一个奇异分布，我们只能从形式上定义一个$\delta$函数。事实上，不存在$g,f*g=f,\forall f$，但很容易构造一个函数列$g_n$，使得$f*g_n\to f$。回忆我们在\ref{sec:convolution}中提到的“平均化”卷积，设$g(x)=\frac{1}{2a}\chi_{[-a,a]}$，则$g*f$是$f$在区间$[x-a,x+a]$上的平均值，即$f*g(t)=\int_{x-a}^{x+a}f(x)\,dx$，$a\to 0,g*f\to f$。
...
\end{align*}
我们希望证明在$\lambda$充分小时，以上两个积分页充分小。以$\int_{0}^{\infty}$为例，由于$f\in PC(\mathbb{R})$，存在$\delta>0$，使得当$|y|<\delta$时，$|f(x-y)-f(x-)|<\epsilon$。于是
\begin{align*}
    \left|\int_{0}^{\delta}[f(x-y)-f(x-)]g_\lambda(y)\,dy\right|\leq &\left|\int_{0}^{\delta}[f(x-y)-f(x-)]g_\lambda(y)\,dy\right|\\
    <   & \epsilon\int_{0}^{\delta}g_\lambda(y)\,dy\\
    =& \left|\int_{0}^{\delta/\lambda}g(y)\,dy\right| \\
    \leq &\delta\int_{0}^{\infty}|g(y)|\,dy
\end{align*}
我们用剩下的条件估计$\int_{\delta}^{\infty}[f(x-y)-f(x-)]g_\lambda(y)\,dy$。如果$g(x)$时限，设$supp\,g\subset[-T,T]$，则当$\lambda<T/\delta$时，$\int_{\delta}^{\infty}[f(x-y)-f(x-)]g_\lambda(y)\,dy=\int_{\delta/\lambda}^{\infty}[f(x-y)-f(x-)]g_\lambda(y)\,dy=0$；如果$f(x)$有界，设$|f(x)|<M$，则
\begin{align*}
...
\end{align*}
$\int_{\delta/\lambda}^{\infty}|g(y)|\,dy\to 0,\delta/\lambda\to\infty$是$\int_{-\infty}^{\infty}|g(y)|\,dy<\infty$的必要条件。
...
\begin{proof}
    将$f$做连续延拓，使得$f\in C(\mathbb{R}),f(x)=0\ for\ x\notin[a-1,b+1]$，则$\lim_{\lambda\to 0}f*G_\lambda \rightrightarrows f,x\in[a,b]$，即左边的极限在区间$[a,b]$上一致收敛到$f$。具体来说，
...
我们当然可以预先求出高斯函数的傅里叶反变换，并且所得结果一定是符合傅里叶反演公式的，具体的做法可以参考\ref{sec:Fourier}中求傅里叶正变换的过程，傅里叶变换的存在性直接由$G_\lambda\in \mathcal{S}$保证，见\ref{sec:Schwartz_Functions}。
\begin{proof}
    考虑
\lr{
...
以上过程利用了高斯函数的速降性质，保证了积分的绝对收敛性，从而可以进行积分换序。我们知道，高斯函数的傅里叶变换仍是高斯函数，具体来说，
\lr{
...
\[\int_{-\infty}^{\infty}f(x)\,dx=\lim_{a\to-\infty,b\to\infty}\int_{a}^{b}f(x)\,dx\neq \lim_{R\to\infty}\int_{-R}^{R}f(x)\,dx=p.v.\int_{-\infty}^{\infty}f(x)\,dx\]
\begin{example}
    设$f(x)=x$，则$\int_{-\infty}^{\infty}f(x)\,dx$发散，但$p.v.\int_{-\infty}^{\infty}f(x)\,dx=0$
\end{example}
\begin{proof}
    我们有\lr{
        &\int_{-R}^{R}e^{2\pi i\xi x}\,d\xi\\
        =&\frac{e^{2\pi i R x}-e^{-2\pi i R x}}{2\pi i x}\\
        =&\frac{\sin(2\pi R x)}{\pi x}
    }{
        &\frac{1}{2\pi}\int_{-R}^{R}e^{i\omega x}\,d\omega\\
        =&\frac{e^{i R x}-e^{-i R x}}{i x}\\
        =&\frac{\sin(R x)}{\pi x}
    }
...
但是，由于$sinc\notin L^1(\mathbb{R})$，无法保证$\lim_{R\to \infty} f(x)*sinc(R x)=f(x)$，因此需要条件$f\in PS(\mathbb{R})$，做更细致的估计：
\lr{
...
取$K$充分大，则以上两积分都是收敛反常积分的尾部，从而是趋于0的。
...
可以想到利用黎曼-勒贝格引理来完成估计。由于$f\in PS(\mathbb{R}),g\in PC(\mathbb{R}^*),g(0+)=f'(x-)$，因此$g$在$[0,K]$上是有界的可积函数，从而$g\in L^1(\mathbb{R})$，根据黎曼-勒贝格引理，$\mathcal{F} g(\pm R)\to 0,R\to\infty$，故$\int_{0}^{K}[f(x-y)-f(x-)]\frac{\sin(R y)}{\pi y}\,dy\to 0,R\to\infty$。得证。
\end{proof}

此外，还有一种基于实分析的傅里叶反演公式，我们仅仅给出其表述：
\begin{theorem}[傅里叶反演公式]
...
首先，可以根据施瓦兹函数类的定义\[\mathcal{S} =\{\varphi\in C^{\infty}(\mathbb{R} ):\lim_{|x|\to\infty}|x|^m\varphi^{(n)}(x)=0,\forall m,n\in\mathbb{N} \}\]给出它们在$|x|\to\infty$时的估计，从而施瓦兹函数$\varphi$必满足$\int_{-\infty}^{\infty}|\varphi(x)|\,dx<\infty$，即施瓦兹函数类是$L^1(\mathbb{R} )$的子集，于是\begin{align*}
...
显然，施瓦兹函数在求导运算下是封闭的，即$\varphi\in\mathcal{S}\Rightarrow \varphi'\in\mathcal{S}$，并且乘以$x$的运算也是封闭的，即$\varphi\in\mathcal{S}\Rightarrow x\varphi\in\mathcal{S}$。利用这两个性质，可以证明施瓦兹函数类在傅里叶变换下的封闭性：
...
\end{align*}
根据黎曼-勒贝格引理（见\ref{sec:approach}），\[\varphi^{(n)}\in L^1(\mathbb{R})\Rightarrow\lim_{|\omega|\to\infty}\mathcal{F}[ \varphi^{(n)}(\omega)]=0\]
...
\end{theorem}
\begin{proof}
我们首先给出一个直观上的说明，再给出两种证明方法。

假设$\mathcal{F} f(t)$支于$[-T/2,T/2]$，我们曾在\ref{sec:Sampling and Interpolation}中证明，$f(t)=\frac{T}{2\pi}Sa(Tt)*f(t)$，又在\ref{sec:convolution}中证明过卷积能够将函数的支集“相加”，故$f(t)$的支集应为$\mathbb{R}$，不是紧支的。要说明积分$Sa(Tt)*f(t)=\int_{-\infty}^{\infty}Sa(Tx)f(t-x)\,dx$在$|t|$充分大时不为0是困难的，但直观上容易接受这个结果。

\textbf{法一}：反证法，假设$suppf\in[-T/2,T/2],supp\mathcal{F} f\in[-\Omega/2,\Omega/2]$，则
\begin{align*}
    f(t)&=\frac{1}{2\pi}\int_{-\infty}^{\infty}\mathcal{F} f(\omega)e^{i\omega t}\,d\omega\\
    &=\frac{1}{2\pi}\int_{-\Omega/2}^{\Omega/2}\mathcal{F} f(\omega)e^{i\omega t}\,d\omega
\end{align*}
由于$suppf\in[-T/2,T/2]$，所以$f(t)$在$|t|>T/2$时恒为0，故\[\int_{-\Omega/2}^{\Omega/2}\mathcal{F} f(\omega)e^{i\omega t}\,d\omega=0,\forall |t|>T/2\]
...
\end{align*}
将$e^{i\omega (t-t_0)}$展开成幂级数，得到\begin{align*}
...
看上去，即便$supp\mathcal{F} f\notin[-\Omega/2,\Omega/2]$，也可以用这种方法证明$f(t)$恒为0。但是，在积分区间为无限区间时，最后一步中的换序一般不成立，因为我们无法证明级数\[\sum_{n=0}^{\infty}\frac{((t-t_0))^n}{n!}(i\omega)^n \mathcal{F} f(\omega)e^{i\omega t_0}\]在$\omega \in (-\infty,\infty)$上一致收敛。

\textbf{法二}：利用解析延拓的唯一性。设$suppf\in[-T/2,T/2]$，则\begin{align*}
    \mathcal{F} f(\omega)&=\int_{-\infty}^{\infty}f(t)e^{-i\omega t}\,dt\\
    &=\int_{-T/2}^{T/2}f(t)e^{-i\omega t}\,dt
\end{align*}
这是一个实变函数，我们可以尝试将其做全纯延拓，不过为了符号的一致性，我们将变量改为$F(i\omega)=\mathcal{F} f(\omega)$，做替换$i\omega\to s=\sigma+i\omega$，得到$f(t)$的拉普拉斯变换：
\begin{align*}
    F(s)&=\int_{-T/2}^{T/2}f(t)e^{-st}\,dt
\end{align*}
由于积分区间有限，$f\in\mathcal{C} $光滑，以上积分总是存在的，并且可以对$s$求导，从而也是全纯的：\[F'(s)=-\int_{-T/2}^{T/2}tf(t)e^{-st}\,dt\]
即$F(s)$是整函数。但是，$F(s)=0,\forall s=i\omega,|\omega|> \Omega/2$，这些点使得$F(s)=0,\forall s\in\mathbb{C}$，这样$f(t)$也只能为0。
\end{proof}

%时域频域“带宽”积，海森堡不确定性原理
必须指出，实际应用中我们总是考虑时域、频域都近似紧支的函数，指出时限、带限不共存，只是理论需要。事实上，时域上的“宽度”与频域上的“宽度”之间存在一个下界，这一点从\ref{sec:Fourier}中提出的伸缩定理就可以看出：
\lr{
    \mathcal{F} [f(at)](\xi)=\frac{1}{|a|}\mathcal{F} f(\frac{\xi}{a})
}{
    \mathcal{F} [f(at)](\omega)=\frac{1}{|a|}\mathcal{F} f(\frac{\omega}{a})
}
随着$a$的增大，时域函数$f(at)$变得越来越集中，而频域函数$\mathcal{F} f(\frac{\xi}{a})$则变得越来越分散，反之亦然。

我们可以用方差来定量描述这一关系。物理规律中许多现象都可以归结为这一数学事实，例如著名的\textbf{海森堡不确定性原理} (Heisenberg's uncertainty principle)。
\begin{theorem}
    时域信号、频域信号的方差之积满足不等式
...
\begin{proof}
    ...
    &=\int_{-\infty}^{\infty}t^2|f(t)|^2\,dt\int_{-\infty}^{\infty}| \mathcal{F} [f'(t)](\xi)|^2\,d\xi\\
    ...
    &\geq \left|\int_{-\infty}^{\infty}tf(t)f'(t)\,dt\right|^2&\text{(三角不等式)}\\
    &=\frac{1}{4}\left|\int_{-\infty}^{\infty}|f(t)|^2\,dt\right|^2&\text{(分部积分)}\\
    &=\frac{1}{4}
    \end{align*}
...
例如，取$a=2\sigma(f)$，则有\[\int_{|t|\geq 2\sigma(f)}|f(t)|^2\,dt\leq \frac{1}{4}\]
即$f$在区间$[-2\sigma(f),2\sigma(f)]$之外的能量不超过总能量的$1/4$，因此前面证明的$\sigma_f \sigma_F\geq 1/4\pi$又可以表述为时域上的“宽度”与频域上的“宽度”之积有下界，游戏可以定义信号的时宽、带宽积：
\begin{definition}[时宽-带宽积]
    设$f\in L^1(\mathbb{R} )\cap L^2(\mathbb{R} )$，$F(\xi)=\int_{-\infty}^{\infty}f(t)e^{-2\pi i\xi t}\,dt$，则$f$的\textbf{时宽}(time width)和\textbf{带宽}(bandwidth)定义分别为
...
\end{itemize}
线性和双射显然成立，下面我们来说明连续性。为此，我们需要给施瓦兹函数类赋予一个拓扑结构。尽管$\mathcal{S}\in L^1(\mathbb{R} )$，但$L^p$空间的范数并不能很好地刻画施瓦兹函数类的性质，因此我们需要引入一族\textbf{半范数} (seminorm)：
\begin{definition}[施瓦兹函数类的半范数]
    对于$m,n\in\mathbb{N} $，定义施瓦兹函数类上的半范数
    \[\|\varphi\|_{m,n}=\sup_{x\in\mathbb{R} }|x|^m|\varphi^{(n)}(x)|\]
\end{definition}
在这个范数下$\mathcal{S} $构成\textbf{弗雷歇空间} (Fréchet space)，即完备的拓扑向量空间。利用这些半范数，我们可以定义施瓦兹函数类中的收敛：
\begin{definition}[施瓦兹函数类中的收敛]
    设$\{\varphi_k\}\subset\mathcal{S} $，如果对于任意$m,n\in\mathbb{N} $，都有
    \[\lim_{k\to\infty}\|\varphi_k-\varphi\|_{m,n}=0\]
...
\begin{proof}
    ...
    \begin{align*}
    \mathcal{F} [((-it)^n\varphi_k)^{(m)}](\omega)&=(i\omega)^m \mathcal{F} [(-it)^n \varphi_k](\omega)\\
    &=(i\omega)^m (\mathcal{F} \varphi_k)^{(n)}(\omega)\\
    \|\mathcal{F} \varphi_k\|_{m,n}&=\sup_{\omega\in\mathbb{R} }|\omega|^m|(\mathcal{F} \varphi_k)^{(n)}(\omega)|\\
    &=\sup_{\omega\in\mathbb{R} }|\mathcal{F} [((-it)^n \varphi_k)^{(m)}](\omega)|
\end{align*}
令$\phi_k=((-it)^n \varphi_k)^{(m)}\in\mathcal{S} $，则\begin{align*}
    \|\mathcal{F} \varphi_k\|_{m,n}&=\sup_{\omega\in\mathbb{R} }|\mathcal{F} \phi_k(\omega)|\leq \int_{-\infty}^{\infty}|\phi_k(t)|\,dt
\end{align*}
...
\begin{align*}
    |\phi_k(t)|&=|((-it)^n \varphi_k)^{(m)}(t)|\\
&=\left|\sum_{j=0}^{m}\binom{m}{j}(t^n)^{(j)}\varphi_k^{(m-j)}(t)\right|\\
    &\leq \sum_{j=0}^{\min(m,n)}\binom{m}{j}\frac{n!}{(n-j)!}|t|^{n-j}|\varphi_k^{(m-j)}(t)|
\end{align*}
由于$\|\varphi_k\|_{n-j+2,m-j}\to 0$，$\forall \epsilon>0,\exists N\in\mathbb{N},k>N\Rightarrow \|\varphi_k\|_{n-j+2,m-j}<\epsilon$，于是
\begin{align*}
...
而在$|t|<1$时，\begin{align*}
    \sum_{j=0}^{\min(m,n)}\binom{m}{j}\frac{n!}{(n-j)!}|t|^{n-j}|\varphi_k^{(m-j)}(t)|&\leq \sum_{j=0}^{\min(m,n)}\binom{m}{j}\frac{n!}{(n-j)!}\|\varphi_k\|_{0,m-j}\\
    &\leq C'\max_{0\leq j\leq \min(m,n)}\|\varphi_k\|_{0,m-j}<\epsilon
\end{align*}
$\phi_k$一致收敛于0，故\[\int_{|t|<1}|\phi_k(t)|\,dt\to 0\]
综上，\[0\leq\|\mathcal{F} \varphi_k\|_{m,n}\leq\int_{-\infty}^{\infty}|\phi_k(t)|\,dt\to 0\]
根据夹逼定理，$\|\mathcal{F} \varphi_k\|_{m,n}\to 0$，得证。傅里叶逆变换同理。
\end{proof}

在弱收敛的意义下，傅里叶变换同样是$\mathcal{T} =\mathcal{S} ' $上的线性拓扑自同构。在\ref{sec:distributions}中，我们已经证明过分布的傅里叶变换的连续性：\begin{theorem}[$\mathcal{T}$上傅里叶变换的连续性]
