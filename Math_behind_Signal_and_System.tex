...
换的运算性质，在实际计算傅里叶变换时，常常不会带入定义计算，而是通过这样的运算性质
来计算。
...
\begin{theorem}[采样函数的“卷积幺元”作用]
    设带限函数$f(t)$满足$supp\mathcal{F} f\subset [-T/2,T/2]$，
...
我们曾在\ref{sec:convolution}中提到，滤波就是把信号中的某些成分剔除或大幅减少，相当于在频域乘以一个函数，在时域上的表现则是与这个函数的傅里叶逆变换进行卷积。从系统函数的视角来看，滤波器就是一个系统，理想滤波器的系统函数在某些频率处接近于0，而在另一些频率处接近于1。根据滤波的目的，滤波器可分为低通滤波器、高通滤波器、带通滤波器和带阻滤波器等。
...
\end{definition}
一般而言，理想滤波器的系统函数是偶函数，因为“时域上是实信号”等价于“频域上是偶函数”，不过偶函数的傅里叶变换仍是偶函数，这将导致系统失去因果性，因此是无法实现的。实际设计滤波器时只能逼近理想滤波器，例如巴特沃斯滤波器、切比雪夫滤波器等。

将$H(\omega)$看作一个复数，即$H(\omega)=|H(\omega)|e^{i\varphi(\omega)}$，则$|H(\omega)|$表示系统对频率为$\omega$的信号成分的放大倍数，$\varphi(\omega)$表示该频率成分经过系统后相位的变化。
...
的统一延迟，即\[r(t)=Ke(t-t_0)\]其中K为常数，$t_0$为常数延迟。由傅里叶变换的时移性质可知，这样的系统函数为
\[H(\omega) = Ke^{-i\omega t_0}\]可以看到，$|H(\omega)|=K,t_0=-\varphi(\omega)/\omega$，其幅频特性是常数，图
像为一水平线；相频特性是线性函数，图像为过原点的直线。

实际上，由于物理限制，我们不可能实现一个无失真传输系统，但只要所讨论的信号的频谱基本都集中在特定的频段当中，我们就可以近似认为在这个频段上满足$|H(\omega)|=K,t_0=-\varphi(\omega)/\omega$的系统是无失真传输系统。

推广时延$t_0=-\varphi(\omega)/\omega$，定义：
...
量带来了剧烈震动。
\end{example}
$\varlimsup_{N \to \infty}S_N^R(t)>1$并不意味着狄利克雷定理失效，因为定理给出的是逐点收敛而非一致收敛，
