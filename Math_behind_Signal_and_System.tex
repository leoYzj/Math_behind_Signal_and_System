...
\end{enumerate}
则称在$V$上定义了一种范数，称$V$为线性赋范空间。特别地，在$\mathbb{C}$ -线性空间$V$上定义范数后，可以自然地诱导出一种度量：
...
\textbf{正交基} (othorgnal bases)指的是内积空间$V$的一组基$\{\mathbf{v_1},\mathbf{v_2},\dots ,\mathbf{v_n}\}$
...
收敛的定义：如果
\[\lim_{n \to \infty} \| g(t)-\sum_{i = 1}^{n}  a_i f_i(t)\|=0\]
就认为级数$\sum_{i = 1}^{n}  a_i f_i(t)$是$g(t)$在这个正交函数系下的分解，
此时记\[g\sim\sum_{n=1}^{\infty}a_n f_n\]
它并不意味着等式右侧的函数项级数在某一点收敛于$g$.在$L^2([0,T])$空间中，我们不
...
最后我们给出一个互相关的应用。设雷达发射了一个信号$f(t)$，经过时间T信号接触到以
物体并发生反射，经过时间T信号回到雷达，雷达接收到的信号$f_r(t)=\alpha (\tau_{2T}f(t))+n(t)$,
...
处取最大值，于是我们只需要观察这个话相关函数的最大值点，就可以得到T。
...
傅里叶反演公式：\lr{
    ...
    f=\frac{1}{2\pi}\mathcal{F} ^{-1}\mathcal{F} f=\frac{1}{2\pi}\mathcal{F}\mathcal{F} ^{-1} f
}
...
\begin{itemize}
    ...
    \item \textbf{平移定理}：\lr{
          ...
          &f(t-b)\overset{\mathcal{F} }{\longleftrightarrow}e^{-i\omega t}F(\omega)\\
...
\textbf{稳态响应}(steady state response)$y_ss(t)$和
\textbf{暂态响应}(transient state)$y_t(t)$，完全解中，$t\to\infty$时保留下来
...
换言之需要利用方程左侧的最高阶导，让方程右侧所需的$\delta$的最高阶导数项出现。
