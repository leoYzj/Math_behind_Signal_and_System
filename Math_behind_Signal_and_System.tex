...
\section{狄拉克梳状函数分布与取样}\label{sec:Sampling and Interpolation}
...
之为\textbf{狄拉克梳状函数}.容易看出，这个分布具有两条基本性质，其一是前
面提到的周期化：$\shah_T*f(t)=f_T(t)$，其二是取样性质，它直接得自$\delta_a$的
取样性质：$\shah_T f(t)=\sum_{k=-\infty}^{\infty}f(kT)\delta_{kT}$。另外，
作为一个分布，有
\[\langle \shah_T,\varphi\rangle=\langle \sum_{k=-\infty}^{\infty}\delta_{kT},\varphi\rangle=\sum_{k=-\infty}^{\infty}\varphi(k_T)\]
...
对于时域上紧支（即在$|t|$充分大时函数恒为0）的函数，级数$\sum_{k=-\infty}^{\infty}f(t-kT)$
实际上是有限和，当然收敛。更为特殊的情况下，函数$f(t)$支于$[-T/2,T/2]$，这时我
们称$f(t)$是\textbf{时限函数}，相应的，在频域上紧支的函数称为\textbf{带限函数}，
不论f定义在哪个域上，在相应的域上周期化所得的函数总是存在，并且有恒等式
\[f(x)=\Pi_{T}(f*\shah_T)(x)\]
这个看似直白的恒等式在傅里叶变换下能够带来一些新的东西。
...
\lr{
    ...
    \mathcal{F} f(k)&=\int_{-\infty}^{\infty}f(t)e^{-ikt}\,dt\\
    &=\int_{-T/2}^{T/2}f(t)e^{-ikt}\,dt\\
    &=Tc_k
}
其中$c_k$是$f(t)$的周期化函数$f_T(t)$的傅里叶系数，也就是说，时限函数的傅里叶变
换在k处的值正比于其周期化函数的第k个傅里叶系数，这使得频域函数$\mathcal{F}f$似乎
在整数处的值蕴含了更多的信息。不仅如此，考虑更一般的函数$f\in\mathcal{S} $，记
...
其中第二行到第三行的积分、求和换序用到了前文中提到的一致收敛性。这个等式意味着对时
...
这个结果称为\textbf{泊松求和公式}(the Poisson summation formula)。

让我们回到一开始的话题，$\shah$分布的傅里叶变换可以借助泊松求和公式得到：
\lr{
...
似乎以$\delta$的速度趋于无穷。这个结果从分析学的角度是必不可能得到的，根据黎曼重
排定理，一个发散的级数可以通过重排求和顺序发散到任意值，而交换1的顺序得到的还是级数
本身，因此我们无法从数学分析的角度估计这个“级数”趋于无穷的阶。实际上我们也只能得到
$\mathbb{R}-\mathbb{Z}$上的收敛性，整数处的分布只能在\ref{sec:distributions}
中提到的弱收敛的意义下定义，即将函数项级数$\sum_{k=-\infty}^{\infty}e^{-2\pi ikx}$的
每一项视为分布，它们作用在任意测试函数$\varphi$上的结果在求和项数充分大时相当于$\delta$
分布作用于$\varphi$的结果。
...
下面讨论\textbf{取样}(sample)。

\section{插值问题}\label{sec:Interpolation}
